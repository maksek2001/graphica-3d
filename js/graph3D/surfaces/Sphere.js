Surfaces.prototype.sphere = (count = 40, R = 7, point = new Point(0, 0, 0), color = null, animation1, animation2, animation3) => {
    let points = [];
    let edges = [];
    let polygons = [];
    const PI = Math.PI;
    ...
    for (let i = 0; i <= PI; i += delta) {
        for (let j = 0; j < 2 * PI; j += delta) {
            ...
            const z = point.z + R * Math.sin(i) * Math.sin(j);
    ...
    // Полигоны    if (color === null) {
        ...
        for (let i = 0; i < points.length; i++) {
            if ((i + 1 + count) < points.length && (i + 1) % count !== 0) {
                polygons.push(new Polygon([i, i + 1, i + 1 + count, i + count], color[i], i))
            } else if ((i + count) < points.length && (i + 1) % count === 0) {
                polygons.push(new Polygon([i, i + 1 - count, i + 1, i + count], color[i], i))
            }
        }
    } else {
        for (let i = 0; i < points.length; i++) {
